...
#2,7,12,17,22,27,32,37,42,47,52,57 * * * * sleep 54 ; /usr/bin/python /$HOME/Dropbox/tRestrictor.py >> /tmp/Killer.log 2>&1 &
# sudo apt-get install python-dateutil # run this on terminal
import email, imaplib #, time, rfc822
import psutil, subprocess, signal, re, string
import datetime as D
from   dateutil import parser, tz
from   RestrictSrc import SrcDict

def Killer(minSince, Dte, playMin=72.):    
    if minSince > playMin: # only kill if playMin is not over 45min in above case
        ...
        for proc in psutil.process_iter():
            if string.lower(proc.name()) == 'java': 
                for each in proc.cmdline:
                    if 'minecraft' in each.lower():
                        print('\tkilling|%s|%s' %(proc.name(), each[:60]))
                        proc.kill()
            if re.search('steam', string.lower(proc.name())) > 0:
                #if proc.username() ==  'hyoon': continue
...
if map(lambda x: string.lower(x.name()), psutil.process_iter()).count('java')  == 0 and \
   map(lambda x: string.lower(x.name()), psutil.process_iter()).count('steam') == 0:
